...
const ll SIZE = 55; // change size before calling
...
void solve()
{
    ...
    // sorting array such that it is increasing order in l and if l are equal then decreasing in r
    sort(all(ranges), myComp);
    vector<int> ans1(n, 0), ans2(n, 0);
 
    int minR = inf;
    for (int i = n - 1;i >= 0;i--)
    {
        if (ranges[i].ff.ss >= minR)
            ans1[ranges[i].ss] = 1;
        minR = min(minR, ranges[i].ff.ss);
    }
 
    int maxR = 0;
    for (int i = 0;i < n;i++)
    {
        if (ranges[i].ff.ss <= maxR)
            ans2[ranges[i].ss] = 1;
        maxR = max(maxR, ranges[i].ff.ss);
    }
    for (int i = 0;i < n;i++)
        cout << ans1[i] << " ";
    cout << "\n";
